...
(function ($) {
    $.fn.extend({ ffform: function (options) {
        ...
        /**********************************************/
        var settings = $.extend({ 'fields': [], onSuccess: null, onFail: null, onValidate: null, submitButton: '', sendingText: 'Sending...', animation: 'flip', validationIndicator: '', successIndicator: '', errorIndicator: '' }, options); return this.each(function () {
            var form = $(this);
            var isValid = true;
            ...
            function init() {
                ...
                successResult = $(settings.successIndicator).text()
                ...
                if (settings.animation == 'flip') {
                    ...
                    flipper.find('.back').append('<span class="lbl" id="msg-close">Close</span>').append(validationIndicator).append(successIndicator).append(errorIndicator);
                }
                else {
                    form.after($('<div class="form-front"></div><div class="form-back"></div>'));
                    form.appendTo($('.form-front'));
                    $('.form-back').append('<span class="lbl" id="msg-close">Close</span>').append(validationIndicator).append(successIndicator).append(errorIndicator);
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        if (successIndicator != null) {
                            $(settings.successIndicator).text(successResult.replace('%result%', result.toString()));
